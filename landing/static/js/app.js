...
// Constantes de etiquetas para ventas y pagos
const METHOD_LABEL = {
...
document.getElementById("signup-form").addEventListener("submit", async e => {
  ...
  const cardNumber = document.getElementById("su-card-number").value;
  const cardExpiry = document.getElementById("su-card-expiry").value;
  const cardCvv = document.getElementById("su-card-cvv").value;
  const cardName = document.getElementById("su-card-name").value;

  // Validación del número de tarjeta
  if (!validateCardNumber(cardNumber)) {
    err.textContent = "Número de tarjeta inválido";
    err.classList.remove("hidden");
    document.getElementById("su-card-number").focus();
    return;
  }

  // Validación de fecha de vencimiento
  if (!validateCardExpiry(cardExpiry)) {
    err.textContent = "Fecha de vencimiento inválida o expirada";
    err.classList.remove("hidden");
    document.getElementById("su-card-expiry").focus();
    return;
  }

  // Validación CVV
  if (!/^\d{3,4}$/.test(cardCvv)) {
    err.textContent = "CVV inválido (debe ser 3 o 4 dígitos)";
    err.classList.remove("hidden");
    document.getElementById("su-card-cvv").focus();
    return;
  }

  // Validación nombre en tarjeta
  if (!cardName.trim() || cardName.length < 3) {
    err.textContent = "Ingresa el nombre como aparece en tu tarjeta";
    err.classList.remove("hidden");
    document.getElementById("su-card-name").focus();
    return;
  }
  ...
  // Separar mes y año de vencimiento (MM/YY)
  ...
  const resp = await fetch("/api/signup", {
    ...
    body: JSON.stringify({
      ...
      // Datos de tarjeta (solo metadata, no número completo)
...
async function loadOrders() {
  ...
  document.getElementById("orders-pending").innerHTML = pending.map(o => `
    <tr>
      <td style="color:var(--text-secondary);font-size:12px">${fmt_dt(o.created_at)}</td>
      <td style="color:var(--text-secondary)">${o.device_name}</td>
      <td><strong style="color:var(--text)">${o.product_name}</strong></td>
      <td><strong style="color:var(--accent2)">$${o.price_usd.toFixed(2)}</strong></td>
      <td><span style="background:var(--surface2);padding:4px 8px;border-radius:6px;font-size:12px">${METHOD_LABEL[o.method] || o.method}</span></td>
      <td style="font-family:monospace;font-size:12px">${o.reference || "—"}</td>
      <td style="color:var(--text-secondary);font-size:12px">${o.buyer_phone || "—"}</td>
      <td style="white-space:nowrap;text-align:right">${canManage ? `
        <button class="btn-sm" style="background:var(--accent2);color:#fff;border:none;margin-right:4px" onclick="approveOrder(${o.id})"><i class="fa-solid fa-check"></i> Aprobar</button>
        <button class="btn-sm" style="background:var(--danger);color:#fff;border:none" onclick="rejectOrder(${o.id})"><i class="fa-solid fa-xmark"></i> Rechazar</button>` : ""}
      </td>
    </tr>`).join("") || '<tr><td colspan="8" style="text-align:center;color:var(--muted);padding:32px"><i class="fa-solid fa-inbox" style="font-size:32px;margin-bottom:12px;display:block;opacity:0.3"></i>No hay pedidos pendientes</td></tr>';

  const hist = orders.filter(o => o.status !== "pending").slice(0, 50);
  document.getElementById("orders-history").innerHTML = hist.map(o => `
    <tr style="${o.status === 'rejected' ? 'opacity:0.6' : ''}">
      <td style="color:var(--text-secondary);font-size:12px">${fmt_dt(o.created_at)}</td>
      <td style="color:var(--text-secondary)">${o.device_name}</td>
      <td style="color:var(--text)">${o.product_name}</td>
      <td><strong style="color:var(--accent2)">$${o.price_usd.toFixed(2)}</strong></td>
      <td><span style="background:var(--surface2);padding:4px 8px;border-radius:6px;font-size:12px">${METHOD_LABEL[o.method] || o.method}</span></td>
      <td style="font-family:monospace;font-size:12px;color:var(--text-secondary)">${o.reference || "—"}</td>
      <td><code style="background:${o.code ? 'var(--accent-glow)' : 'var(--surface2)'};color:${o.code ? 'var(--accent)' : 'var(--muted)'};padding:5px 10px;border-radius:6px;font-weight:600;letter-spacing:1px">${o.code || "—"}</code></td>
      <td>${PAY_STATUS[o.status] || o.status}</td>
    </tr>`).join("") || '<tr><td colspan="8" style="text-align:center;color:var(--muted);padding:32px"><i class="fa-solid fa-receipt" style="font-size:32px;margin-bottom:12px;display:block;opacity:0.3"></i>Sin ventas todavía</td></tr>';
}
...
function formatDuration(mins) {
  if (mins >= 1440) return `${mins/1440} día${mins/1440 > 1 ? 's' : ''}`;
  if (mins >= 60) return `${mins/60} hora${mins/60 > 1 ? 's' : ''}`;
  return `${mins} min`;
}
...
async function loadProducts() {
  ...
  document.getElementById("products-table").innerHTML = prods.map(p => `
    <tr style="${p.is_active ? "" : "opacity:.5"}">
      ...
      <td style="color:var(--text-secondary)">${formatDuration(p.duration_min)}</td>
      <td><strong style="color:var(--accent2)">$${p.price_usd.toFixed(2)}</strong></td>
      <td style="color:var(--text-secondary)">${p.price_ves > 0 ? "Bs. " + p.price_ves.toFixed(2) : "—"}</td>
      <td>${p.is_active ? '<span class="badge badge-green"><i class="fa-solid fa-circle-check"></i> Activo</span>' : '<span class="badge badge-gray"><i class="fa-solid fa-circle-xmark"></i> Inactivo</span>'}</td>
      <td style="white-space:nowrap;text-align:right">
        <button class="btn-sm" onclick="toggleProduct(${p.id}, ${!p.is_active})" style="min-width:90px">
          <i class="fa-solid fa-${p.is_active ? 'eye-slash' : 'eye'}"></i> ${p.is_active ? "Desactivar" : "Activar"}
        </button>
      </td>
    </tr>`).join("") || '<tr><td colspan="6" style="text-align:center;color:var(--muted);padding:32px"><i class="fa-solid fa-box-open" style="font-size:32px;margin-bottom:12px;display:block;opacity:0.3"></i>Crea tu primer producto para activar la tienda</td></tr>';
}
...
function showRegisterModal() {
  ...
  document.getElementById("activation-code-panel").classList.remove("hidden");
  document.getElementById("activation-manual-panel").classList.add("hidden");
  document.querySelectorAll(".activation-tab").forEach(t => t.classList.remove("active"));
  document.querySelector('[data-method="code"]').classList.add("active");

  // Clear forms
  if (document.getElementById("activate-form")) document.getElementById("activate-form").reset();
...
function switchActivationMethod(method) {
  // Update tabs
  document.querySelectorAll(".activation-tab").forEach(t => t.classList.remove("active"));
  ...
  if (method === "code") {
    ...
    document.getElementById("activation-code-panel").classList.add("hidden");
    document.getElementById("activation-manual-panel").classList.remove("hidden");
  }
}

// Activación rápida por código
document.getElementById("activate-form").addEventListener("submit", async e => {
  ...
  const location = document.getElementById("activate-location").value.trim();

  // Usar endpoint de activación
  const resp = await fetch("/api/devices/activate", {
    method: "POST",
  ...
  if (resp.ok) {
    ...
    // Mostrar resultado de activación
    document.getElementById("activation-code-panel").classList.add("hidden");
    document.getElementById("activation-manual-panel").classList.add("hidden");
    document.getElementById("activation-success-info").classList.remove("hidden");
    document.getElementById("manual-success-info").classList.add("hidden");

    document.getElementById("activated-name").textContent = name;
    document.getElementById("activated-location").textContent = location || "Sin ubicación";
    document.getElementById("success-message").textContent = "Tu gateway JADSlink está activado y listo para usar";
    ...
    const err = await resp.json().catch(() => ({}));
    errEl.textContent = err.detail || "Código inválido o gateway ya activado";
...
document.getElementById("register-form").addEventListener("submit", async e => {
  ...
  if (resp.ok) {
    ...
    document.getElementById("activation-code-panel").classList.add("hidden");
    document.getElementById("activation-manual-panel").classList.add("hidden");
    document.getElementById("activation-success-info").classList.add("hidden");
    document.getElementById("manual-success-info").classList.remove("hidden");

    document.getElementById("success-message").textContent = "Gateway registrado. Configura el agente en tu router";
    document.getElementById("register-result").classList.remove("hidden");
...
function closeRegisterModal() {
  ...
  // Refrescar la vista de dispositivos
  setTimeout(() => {
    const tab = document.querySelector('.nav-item[data-tab="devices"]');
    if (tab) tab.click();
  }, 100);
}
...
function initTheme() {
  const saved = localStorage.getItem("hcm_theme");
...
function updateThemeIcon(theme) {
  ...
  if (icon) {
    // Sol cuando oscuro (indica que puedes cambiar a claro), luna cuando claro
...
function formatCardNumber(input) {
  ...
  brandIcon.className = 'card-brand';
...
// Calculate first charge date (14 days from now)
function updateFirstChargeDate() {
  const dateEl = document.getElementById('first-charge-date');
  if (!dateEl) return;
