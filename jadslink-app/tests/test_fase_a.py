...
def h(pw):  # hash legacy (sha256 plano) tal como lo guardaban las BDs previas
    return hashlib.sha256(pw.encode()).hexdigest()


def test_fase_a(fresh_db):
    DB = fresh_db
    ...
    with TestClient(app) as client:
        ...
        # tras el login, el hash legacy fue re-hasheado a pbkdf2 y el login sigue OK
        con = sqlite3.connect(DB)
        new_hash = con.execute("SELECT password_hash FROM users WHERE username='admin'").fetchone()[0]
        con.close()
        assert new_hash.startswith("pbkdf2$"), f"el hash legacy debía migrar a pbkdf2: {new_hash[:20]}"
        assert client.post("/api/auth/login", json={"username":"admin","password":"secretoViejo"}).status_code == 200
        print("✓ Re-hash transparente sha256 → pbkdf2 en el primer login")
