...
class UpgradeRequest(BaseModel):
    ...
    ticket_quantity: Mapped[Optional[int]] = mapped_column(Integer, nullable=True)

    # Para plan_upgrade: a qué plan
    new_plan_tier: Mapped[Optional[str]] = mapped_column(String(50), nullable=True)
    ...
    amount_vef: Mapped[Optional[Decimal]] = mapped_column(Numeric(14, 2), nullable=True)
    exchange_rate: Mapped[Optional[Decimal]] = mapped_column(Numeric(10, 4), nullable=True)  # Tasa oficial del día
    ...
    payment_details: Mapped[Optional[Dict[str, Any]]] = mapped_column(JSON, nullable=True)  # JSON con datos específicos

    # Para Pago Móvil
    banco_origen: Mapped[Optional[str]] = mapped_column(String(100), nullable=True)
    cédula_pagador: Mapped[Optional[str]] = mapped_column(String(20), nullable=True)
    referencia_pago: Mapped[Optional[str]] = mapped_column(String(50), nullable=True)
    comprobante_url: Mapped[Optional[str]] = mapped_column(String(500), nullable=True)

    # Para Tarjeta
    últimos_4_digitos: Mapped[Optional[str]] = mapped_column(String(4), nullable=True)

    # Notas y auditoría
    admin_notes: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    rejection_reason: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    approved_by: Mapped[Optional[str]] = mapped_column(String(255), nullable=True)  # Email del admin

    # Recordatorios
    reminder_count: Mapped[int] = mapped_column(Integer, default=0, nullable=False)
    last_reminder_at: Mapped[Optional[str]] = mapped_column(String(255), nullable=True)
